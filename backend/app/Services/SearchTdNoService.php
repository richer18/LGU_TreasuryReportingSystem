...
class SearchTdNoService
{
    public function search(string $tdNo, int $limit = 100): array
    {
        return $this->run(['--td-no', $tdNo, '--limit', (string) $limit]);
    }

    private function run(array $arguments): array
    {
        $script = config('firebird.search_td_no_script');

        if (! is_file($script)) {
            return [
                'ok' => false,
                'error' => 'Search TD No. runner script was not found.',
                'script' => $script,
            ];
        }
